...
def random_obstacle_map(xs=25, ys=25, p = 0.2, minlen=2):
    """Returns a 2D 'map' with a rectangle building centered in the middle.
    ...
    def open_closed_array(rmap_matrix):
        for i in range(1,rmap_matrix.shape[0]-1):
            for j in range(1,rmap_matrix.shape[1]-1):
                if np.sum(rmap_matrix[i-1:i+2,j-1:j+2])<-7: #surrounded by -1
                    if np.random.randint(2):
                        rmap_matrix[i-1:i+2,:] = 0
                    else:
                        rmap_matrix[:,j-1:j+2] = 0
        return rmap_matrix
    rmap_original = np.zeros((xs+2, ys+2), dtype=np.int32)

    # rmap_original = np.ones((xs+2, ys+2), dtype=np.int32)*(-1)
    # rmap_original[0,1:-1] = 0
    rmap = np.zeros((xs, ys), dtype=np.int32)

    # number of obstacle blocks
    len_obstacle = int(int(xs*ys*p)/minlen)
    if len_obstacle > 0:
        x_obs = np.random.randint(len_obstacle)
        for i in range(x_obs):
            xloc = np.random.randint(xs)
            yloc = np.random.randint(ys-minlen+1)
            rmap[xloc,yloc:yloc+minlen] = -1
    else:
        x_obs = 0

    y_obs =  int(len_obstacle-x_obs)
    if y_obs > 0:
        for i in range(y_obs):
            xloc = np.random.randint(xs-minlen+1)
            yloc = np.random.randint(ys)
            rmap[xloc:xloc+minlen,yloc] = -1

    rmap_original[1:-1, 1:-1] = rmap
    rmap_original = open_closed_array(rmap_original)

    return rmap_original
