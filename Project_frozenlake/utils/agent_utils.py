...
def create_agents_by_goals(
    ...
    expanded_mat: This matrix is used to spawn non-adjacent agents
    """
    goal_state = model_state[2]
    xs, ys = model_state[0].shape
    agents = []
    expanded_mat = np.zeros((xs + 2, ys + 2))
    for i in range(nagents):
        # TODO: 이거는 그냥 0으로 하는데 이게 아니라 서로 겹치지 않는 위치에 둬야할거같음
        valid = False
        while not valid:
            xinit, yinit = np.random.randint(xs), np.random.randint(ys)
        xinit, yinit = (0, 0)
        if randinit:
            xinit, yinit = feasible_position_wgoal_exp(
                randomizer, model_state[0], expanded_mat, constraints=constraints
            )
            # fill expanded_mat
            expanded_mat[xinit + 1, yinit + 1] = -1
            expanded_mat[xinit + 2, yinit + 1] = -1
            ...
            expanded_mat[xinit + 1, yinit] = -1
        agent = DiscreteAgent(
            xs, ys, model_state[0], randomizer, obs_range=obs_range, flatten=flatten
        )
        agent.set_position(xinit, yinit)
        agents.append(agent)
    return agents
...
def feasible_position_wgoal_exp(randomizer, map_matrix, expanded_mat, constraints=None):
    """Returns a feasible position on map (map_matrix)."""
    xs, ys = map_matrix.shape
    while True:
        if constraints is None:
            ...
            x = randomizer.integers(xl, xu)
            y = randomizer.integers(yl, yu)
        #TODO 이것도 약간 다른 agent 위치 와 겹치지 않게
        if map_matrix[x, y] != -1 and expanded_mat[x + 1, y + 1] != -1:
            return (x, y)
